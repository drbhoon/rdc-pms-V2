 ...
 * Headers: x-pms-user
 */
import { submitBhAssessment } from '../../../lib/workflow';
...
export default async function handler(req, res) {
  ...
  try {
    const result = await submitBhAssessment(roleKey, pairId, values, user);
    return res.status(200).json({ success: true, ...result });
  } catch (err) {
    console.error('[bh-submit]', err);
